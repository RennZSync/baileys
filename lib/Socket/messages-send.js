...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, isSecret, protected: protectedSend, me: meOnly, noSelfSync, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        const extraAttrs = {};
        if ((isSecret || protectedSend || meOnly || noSelfSync) && !isGroup && !isStatus) {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                for (const { user, jid } of devices) {
                    ...
                    // noSelfSync: don't fan the message out to the sender's own other devices,
                    // so it never shows up (synced) on the sender's phone/companions.
                    if (noSelfSync && isMe) {
                        continue;
                    }
                    if (isMe) {
            ...
            // --- Porting dari ourin-baileys 9.0.21 (ob9) ---
            // Auto-inject node <biz> untuk pesan bertombol. Dilakukan hanya bila
            // caller belum menyertakan node biz sendiri (additionalNodes).
    ...
    // --- Porting dari ourin-baileys 9.0.21 (ob9) ---
    // Deteksi tipe tombol pada pesan untuk injection node <biz> di stanza.
    // Tanpa node <biz>, server menerima stanza tapi kartu tombol/list tidak
    // dirender client penerima (relayMessage resolve, pesan tak muncul).
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                await relayMessage(jid, fullMsg.message, {
                    ...
                    me: options.me,
                    noSelfSync: options.noSelfSync === true
                });
